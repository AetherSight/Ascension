...
def build_gallery(
    ...
    """
    Build gallery embeddings from one or multiple roots.

    Args:
        gallery_roots: str or list of str, each a root folder with class subfolders
    """
    if isinstance(gallery_roots, str):
...
def extract_patches_from_image(img, patch_transform, model, device, num_patches=5):
    ...
    Uses fixed center-based patch positions for consistency.
    """
    patch_size = 224
    H, W = img.shape[:2]
...
def evaluate_real_world_images(
    ...
    """
    Evaluate real-world images against a gallery.

    Args:
        gallery_roots: str or list of str, gallery roots containing class folders
        patch_weight: weight for patch features. If 0, patch matching is disabled.
        patch_only: if True, treat input as a single patch (no extraction).
    """
    if patch_only:
        global_weight = 0.0
        patch_weight = 1.0
    ...
    num_patches = 5  # fixed number when extracting from full image
...
def verify_real_world_image(
    ...
    Args:
        ...
        gallery_roots: Gallery root directory or list of roots
...
if __name__ == "__main__":
    ...
    verify_real_world_image(
        ...
        gallery_roots=[r"S:\\FFXIV_train_dataset", r"S:\\FFXIV_train_dataset2"],
