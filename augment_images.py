...
class ClothingDataset(Dataset):
    """
    服装数据集类，使用 A2ClothingTransform 进行数据增强
    """
    def __init__(self, image_paths, labels=None, transform=None, is_train=True):
        ...
        Args:
            ...
            transform: A2ClothingTransform 实例，如果为 None 则自动创建
        ...
        if transform is None:
            self.transform = A2ClothingTransform(train=is_train)
        else:
...
def create_dataloader(image_paths, labels=None, batch_size=32, shuffle=True, 
    ...
    Args:
        ...
        is_train: 是否为训练模式
        transform: 自定义 transform，如果为 None 则使用 A2ClothingTransform
...
def preview_augmentations(image_path, grid_size=(5, 5), output_path=None, show=True):
    ...
    # 复用 A2ClothingTransform，设置 return_tensor=False 用于预览
    transform = A2ClothingTransform(train=True, return_tensor=False)
